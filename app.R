...
zebrafish <- dbPool(RSQLite::SQLite(), dbname = "zfish3.sqlite")
...
ui <- fluidPage(
  ...
  br(),
  fluidRow(width="100%",
           DT::DTOutput("responses_table", width = "100%")
  ), 
  fluidRow(width="25%", 
...
server <- function(input, output, session) {
  ...
  deleteData <- reactive({
    ...
    SQL_df <- dbReadTable(zebrafish, "responses_df")
    print(input$responses_table_rows_selected)
    ...
    showModal(
     if(length(input$responses_table_rows_selected)>=1 ){
       modalDialog(
         ...
         paste("Do you want to delete?"),
         footer = tagList(
           modalButton("No"),
           actionButton("confirm_delete", "Yes")
         )
       )
     }else if (length(input$responses_table_rows_selected) < 1 ){
          modalDialog(
  ...
  output$responses_table <- DT::renderDataTable({
    ...
    res.table <- responses_df()
    res.table <- res.table[,c(1:12)]
    names(res.table) <- c("Batch", "Name", "Birthday","Death", "Status", "User", "Strain", "Generation",
                      "Adults", "Nursery","Mother", "Father")
    res.table <- datatable(res.table, 
                       rownames = FALSE,
  ...
  #outputOptions(output, "responses_table", suspendWhenHidden = FALSE)
